...
void MyBarrier()
{
	...
	startCount--;								// decrement active count when the thread is completed
	if (startCount==0)
	{	// last thread to enter the barrier
		...
		startCount=nThreads; 					// reset active count for next barrier
	}
	else
	{
		pthread_mutex_unlock(&startCountMutex);	// spinning
	}
...
void* Transform2DTHread(void* v)
{
	...
	Transform1D(h,N,W,Wconj);
	// cout<<"Process "<<myID<<" completed\n";

	MyBarrier();	// All threads enter the barrier after being created and wait for everyone to finish
	return 0;
}
...
{ 
  ...
  cout<<"1D FFT started"<<endl;
  createThreads(nThreads);		// create threads to compute 1D FFT row-wise
  pthread_cond_wait(&exitCond, &exitMutex);	// wait till all threads have exited the barrier
 
  image.SaveImageData("MyAfter1D.txt",data,wd,ht);	//save 1D FFT results
  cout<<"1D FFT completed\n"<<endl;

  Transpose(N,data);			// Transpose results of 1D FFT to compute column-wise transform

  cout<<"2D FFT started"<<endl;
  createThreads(nThreads);	// create threads to compute 1D FFT column-wise
  pthread_cond_wait(&exitCond, &exitMutex);	// wait till all threads have exited the barrier
...
void Transform2dInverse(const char* inputFN) 
{
  ...
  image.SaveImageData("MyAfterInverse.txt",data,wd,ht);	//save 2D FFT results
